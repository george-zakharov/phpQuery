...
//				$this->elements = $stack;
            ...
            case 'parent':
                $this->elements = $this->map(
                    function ($node) {return $node instanceof DOMELEMENT && $node->childNodes->length
                        ? $node : null;}
                )->elements;
                break;
            case 'empty':
            ...
            case 'enabled':
                $this->elements = $this->map(
                    function ($node) {return pq($node)->not(":disabled") ? $node : null;}
                )->elements;
                break;
            case 'header':
                $this->elements = $this->map(
                    function ($node) {$isHeader = isset($node->tagName) && in_array($node->tagName, array(
...
//									: null;
                    ...
                    // index
                    $mapped = $this->map(
                        function ($node, $index) {$prevs = pq($node)->prevAll()->size();
